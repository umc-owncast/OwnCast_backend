...
public interface CastRepository extends JpaRepository<Cast, Long> {
    ...
    @Query("SELECT c FROM Cast c " +
            ...
            "WHERE m.mainCategory.id = :mainCategoryId AND c.isPublic = true " +
            "ORDER BY c.hits DESC")
    Page<Cast> findTop5ByMainCategoryIdOrderByHitsDesc(@Param("mainCategoryId") Long mainCategoryId, @Param("pageable") Pageable pageable);
}
